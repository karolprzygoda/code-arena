...
import TestCaseFormField from "@/app/(root)/(challenge)/(manage-challenge)/_components/test-case-form-field";
...
import MarkdownBuilder from "@/app/(root)/(challenge)/(manage-challenge)/_components/markdown-editor/markdown-builder";
...
const ChallengeForm = ({
  ...
  const { getMarkdown } = useMarkdownContext(
    useShallow((state) => ({
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    form.setValue("description", getMarkdown());
    form.handleSubmit(onSubmit, onError)();
  };

  if (isEditingDescription) {
    return <MarkdownBuilder className={"lg:w-1/2"} />;
  }
